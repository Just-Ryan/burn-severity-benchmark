...
& The segmentation models were trained earlier on the collection's original split: 179 of 205 internal test images (87.3\%) lie in their training or validation folds. Retraining both models on the source-grouped split, changing nothing but the partition, drops the standalone model from 90.7\% to \textbf{78.0\%} on the same images: \textbf{the leak was worth 12.7 pp}. All external numbers, and every result computed from ground-truth masks, are unaffected (verified, zero overlap)
& \ref{Res:segleak} \\
\hline
\end{tabular}
\label{tab:audit}

...
\ShortHeadings{Five evaluation choices that changed our conclusions}{Altayeb, Alraddadi and Alrehaili}
...
\title{Deep learning for burn severity assessment: five evaluation choices
...
\label{Mat:ita}
Because any imaging aid intended for clinical use should be checked for systematic dependence on skin tone, we added an exploratory probe. No ground-truth phototype labels exist for either dataset, so we estimated \emph{apparent} skin tone from the images with the Individual Typology Angle, $\mathrm{ITA}=\arctan((L^{*}-50)/b^{*})\cdot 180/\pi$, computed in CIE $L^{*}a^{*}b^{*}$ under a D65 white point and binned into the six conventional bands (very light, light, intermediate, tan, brown, dark).
...
Standalone YOLOv8-seg (single seed, conf.\ 0.05) & $\mathbf{78.0}$ (bal.\ $77.7$) & $48.9$ (bal.\ $50.3$) \\
\quad same model before the split was corrected & \emph{(90.7)} & --- \\
\hline
